...
public class WiredConditionWearsEffect extends InteractionWiredCondition {
    ...
    public WiredConditionWearsEffect(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
    }

    public WiredConditionWearsEffect(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    protected boolean matchesCondition(WiredContext ctx) {
        ...
        if (this.quantifier == QUANTIFIER_ALL) {
            return users.stream().allMatch(u -> u != null && u.getEffectId() == this.effectId);
        }
        return users.stream().anyMatch(u -> u != null && u.getEffectId() == this.effectId);
    }
    ...
    public String getWiredData() {
        return WiredManager.getGson().toJson(new JsonData(this.effectId, this.quantifier, this.userSource));
    }
    ...
    public void loadWiredData(ResultSet set, Room room) throws SQLException {
        ...
        if (wiredData != null && wiredData.startsWith("{")) {
            ...
            if (data != null) {
                this.effectId   = Math.max(0, data.effectId);
    ...
    public boolean saveData(WiredSettings settings) {
        ...
        this.effectId   = (p.length > 0) ? Math.max(0, p[0])                                            : 0;
    ...
    static class JsonData {
        ...
        public JsonData(int effectId, int quantifier, int userSource) {

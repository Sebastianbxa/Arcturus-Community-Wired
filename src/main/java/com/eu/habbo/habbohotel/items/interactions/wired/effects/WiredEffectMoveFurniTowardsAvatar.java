...
public class WiredEffectMoveFurniTowardsAvatar extends InteractionWiredEffect {
    public static final WiredEffectType type = WiredEffectType.MOVE_FURNI_TOWARDS_AVATAR;

    private THashSet<HabboItem> items = new THashSet<>();

    public WiredEffectMoveFurniTowardsAvatar(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
    }

    public WiredEffectMoveFurniTowardsAvatar(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    public void execute(WiredContext ctx) {
        ...
        try {
            for (HabboItem item : sourceItems) {
                ...
                if (target != null) {
                    int[] step = this.calculateStepTowards(item.getX(), item.getY(), target.getX(), target.getY());
                    int x = step[0];
                    int y = step[1];
    ...
    public boolean simulate(WiredContext ctx, WiredSimulation simulation) {
        ...
        for (HabboItem item : this.resolveSourceItems(ctx, this.items)) {
            ...
            if (target != null && target.getCurrentLocation().distance(t) > 1) {
                int[] step = this.calculateStepTowards(currentPos.x, currentPos.y, target.getX(), target.getY());
                int x = step[0];
                int y = step[1];
    ...
    private int[] calculateStepTowards(int itemX, int itemY, int targetX, int targetY) {
        int deltaX = targetX - itemX;
        int deltaY = targetY - itemY;

        if (Math.abs(deltaX) > Math.abs(deltaY)) {
            return new int[] { Integer.compare(deltaX, 0), 0 };
        }

        return new int[] { 0, Integer.compare(deltaY, 0) };
    }

...
public class WiredEffectSendSignalNegative extends InteractionWiredEffect {
    public static final WiredEffectType type = WiredEffectType.NEGATIVE_SEND_SIGNAL;
    ...
    public WiredEffectSendSignalNegative(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
        ...
        for (HabboItem antenna : antennas) {
            for (List<HabboItem> itemPayload : itemPayloads) {
                for (List<RoomUnit> userPayload : userPayloads) {
                    WiredState signalState = ctx.state().fork();
